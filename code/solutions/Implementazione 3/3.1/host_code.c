...
cl_event sort( 
			  ...
			  cl_int n_sott, cl_int max_sott_dimm
	...
	err = clSetKernelArg(parallel_quick, 5, sizeof(max_sott_dimm), &max_sott_dimm);
	ocl_check(err, "set vecinit arg 5");

	size_t round_lws = round_mul_up (max_sott_dimm,4);

	//Calcolo il Global Work Size
	const size_t gws[] = { (round_lws/4)*n_sott };

	const size_t lws[] = { (round_lws/4)  };
...
int find_next_gws (cl_command_queue que, cl_mem delimiters, int dim, cl_mem map) {
	...
	// Se l'if sopra non va a buon fine, calcolo la mappa come al solito
	if (flag == 0) {
...
int main (int argc, char *argv[]) {
	...
	//Numero di sottoproblemi da risolvere
	int n_sott = 1;
	...
	while (n_sott>0) {
		...
		if (flag == 1) {
			ev[n_iterazioni]= sort (que, parallel_q, d_v1, smaller_pivot, greater_pivot, delimiters, app, 0, NULL,n_sott, max_sott_size);
